...
public class SecurityConfig {
	...
	public SecurityFilterChain secutityFilterChain(HttpSecurity http) throws Exception {
		...
		.authorizeHttpRequests(auth -> auth
			    ...
			    .requestMatchers("/auth/user/all", "/auth/user/ban/**", "/auth/user/approve/**").hasAuthority("ADMIN")
			    .requestMatchers("/admin/**").hasAuthority("ADMIN")
			    .requestMatchers("/auth/user/profile", "/auth/user/password", "/auth/user/deactivate").authenticated()
			    .requestMatchers(
			    	    "/v3/api-docs/**",
			    	    "/swagger-ui/**",
			    	    "/swagger-ui.html"
